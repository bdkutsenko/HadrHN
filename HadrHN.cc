...
int main(int argc, char** argv)
{
  ...
  std::vector<G4String> gp_mod_names = {"CHIPS","XS","ISOCHIPS","ISOXS"};
  ...
  if(name == "ISOXS" || name == "ISOCHIPS") ss->SetCurrentCouple(isocouple);
  ...
  for (i=0; i<nbins; ++i) {
	...
	dParticle.SetKineticEnergy(e);
      } else if (idx == 6 && (name == "ISOCHIPS" || name == "ISOXS")){
